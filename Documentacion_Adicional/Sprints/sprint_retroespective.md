# 📋 **Sprint Retrospective: Sistema Reserva de Aulas**

| Dato | Detalle |
| :--- | :--- |
| **Sprint** | #1 |
| **Fecha** | 2026-03-18 |
| **Scrum Master** | Luis Abreo |
| **Participantes** | (DEV.) Jesús Sanchez, (DEV.) Sebastian Rolon, (PO.) Juan Rosales|
| **Estado del Sprint** | 🟢 Completado

---

## 1. 🎯 **Objetivo de la Sesión**
Analizamos el desempeño del equipo durante el último Sprint para identificar qué procesos, herramientas o interacciones deben mantenerse, eliminarse o evolucionar, respecto al sistema de reservas de salas que hemos implementado.

## 2. 📊 **Resumen de Ejecución**
* **Historias de Usuario:** [HU-01, HU-02, HU-03, HU-04]
* **Cumplimiento de historias:** Completadas
* **Incidencias Humanas:** El Developtment Team sufrio un cambio de personal a mitad del proceso, debido a la ausencia de uno de los developers
* **Incidencias Técnicas:** El sistema no pudo compilarse en Linux debido a incongruencia de versiones

---

## 3. 🧠 **Dinámica de Reflexión**

* ### ✅ ¿Qué salió bien?
    * El diseño inicial de la interfaz facilito la creacion de las otras funciones. 
    * El acoplamiento del nuevo miembro en el Dia 2 del Sprint fue oportuno
    * La comunicacion y el trabajo en equipo entre los miembros fue efectiva
* ### ❌ ¿Qué no salió tan bien?
    * Ausencia inesperada de un desarrollador
* ### 🚀 ¿Qué podemos mejorar?
    * Implementar GitFlow
* ### 💡 ¿Qué aprendimos?
    * Trabajo en equipo con comunicacion asertiva y escucha activa para poder desarrollar un desarrollo de software colaborativo
