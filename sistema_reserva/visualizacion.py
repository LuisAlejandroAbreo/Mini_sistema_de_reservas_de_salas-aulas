...
class VentanaPrincipal(tk.Tk):
    ...
    def _abrir_modal_crear_reserva(self):
        ...
        tk.Label(inner, text="Fecha *", font=("Segoe UI", 10, "bold"),
                 bg=C["bg_card"], fg=C["text_2"], anchor="w").pack(anchor="w", pady=(10, 3))
        ...
        _hoy         = _dt.date.today()
        _cal_year    = [_hoy.year]
        _cal_month   = [_hoy.month]
        _fecha_sel   = [None]          # almacena datetime.date seleccionada

        # Contenedor del calendario
        cal_wrap = tk.Frame(inner, bg=C["bg_card"],
                            highlightbackground=C["border"], highlightthickness=1)
        cal_wrap.pack(fill="x", pady=(0, 4))

        # Cabecera mes/año con navegación
        nav = tk.Frame(cal_wrap, bg=C["primary"])
        nav.pack(fill="x")

        lbl_mes_anio = tk.Label(nav, text="", font=("Segoe UI", 9, "bold"),
                                bg=C["primary"], fg="white")
        ...
        def _btn_nav(parent, txt, cmd):
            b = tk.Label(parent, text=txt, font=("Segoe UI", 10, "bold"),
                         ...
                         padx=12, pady=4)
            b.pack(side="right" if txt == "▶" else "left")
            b.bind("<Button-1>", lambda e: cmd())
            b.bind("<Enter>", lambda e: b.config(bg=C["primary_dark"]))
            b.bind("<Leave>", lambda e: b.config(bg=C["primary"]))

        # Grid días
        grid_frame = tk.Frame(cal_wrap, bg=C["bg_card"])
        grid_frame.pack(fill="x", padx=4, pady=4)

        _dia_btns = {}   # (row, col) → Label

        DIAS_SEM = ["Lu", "Ma", "Mi", "Ju", "Vi", "Sa", "Do"]
        for ci, d in enumerate(DIAS_SEM):
            tk.Label(grid_frame, text=d, font=("Segoe UI", 8, "bold"),
                     bg=C["bg_card"], fg=C["text_3"],
                     width=4, anchor="center").grid(row=0, column=ci, pady=(2, 4))

        for ri in range(1, 7):
            for ci in range(7):
                lbl = tk.Label(grid_frame, text="", font=("Segoe UI", 9),
                               bg=C["bg_card"], fg=C["text_1"],
                               width=4, cursor="hand2", pady=3)
                lbl.grid(row=ri, column=ci)
                _dia_btns[(ri, ci)] = lbl

        def _renderizar_calendario():
            y, m = _cal_year[0], _cal_month[0]
            MESES_ES = ["Enero","Febrero","Marzo","Abril","Mayo","Junio",
                        "Julio","Agosto","Septiembre","Octubre","Noviembre","Diciembre"]
            lbl_mes_anio.config(text=f"  {MESES_ES[m-1]}  {y}  ")

            # Limpiar todos los botones
            for lbl in _dia_btns.values():
                lbl.config(text="", bg=C["bg_card"], fg=C["text_1"],
                           cursor="arrow")
                lbl.unbind("<Button-1>")
                lbl.unbind("<Enter>")
                lbl.unbind("<Leave>")

            cal_matrix = _cal.monthcalendar(y, m)
            hoy = _dt.date.today()

            for ri, semana in enumerate(cal_matrix, start=1):
                for ci, dia in enumerate(semana):
                    if dia == 0:
                        continue
                    fecha = _dt.date(y, m, dia)
                    lbl   = _dia_btns[(ri, ci)]

                    # Colores
                    es_sel  = (_fecha_sel[0] == fecha)
                    es_hoy  = (fecha == hoy)
                    pasado  = (fecha < hoy)
                    ...
                    if es_sel:
                        ...
                        bg_d, fg_d = C["bg_card"], C["text_3"]
                    else:
                        bg_d, fg_d = C["bg_card"], C["text_1"]

                    lbl.config(text=str(dia), bg=bg_d, fg=fg_d,
                               cursor="arrow" if pasado else "hand2")

                    if not pasado:
                        def _click(e, f=fecha):
                            _fecha_sel[0] = f
                            _renderizar_calendario()
                        lbl.bind("<Button-1>", _click)
                        if not es_sel:
                            lbl.bind("<Enter>",
                                     lambda e, l=lbl, bg=bg_d: l.config(bg=C["border"]))
                            lbl.bind("<Leave>",
                                     lambda e, l=lbl, bg=bg_d: l.config(bg=bg))

        def _mes_anterior():
            if _cal_month[0] == 1:
                _cal_month[0] = 12; _cal_year[0] -= 1
            else:
                _cal_month[0] -= 1
            _renderizar_calendario()
        ...
        def _mes_siguiente():
            if _cal_month[0] == 12:
                _cal_month[0] = 1; _cal_year[0] += 1
            else:
                _cal_month[0] += 1
            _renderizar_calendario()

        _btn_nav(nav, "◀", _mes_anterior)
        _btn_nav(nav, "▶", _mes_siguiente)
        _renderizar_calendario()

        # ── Hora inicio / fin — Combobox de horas exactas ─
        ...
        tk.Label(fila_hora, text="Inicio", font=("Segoe UI", 9),
                 bg=C["bg_card"], fg=C["text_3"]).pack(side="left")

        var_inicio = tk.StringVar(value=HORAS[2])   # 08:00 por defecto
        combo_inicio = ttk.Combobox(fila_hora, textvariable=var_inicio,
                                    values=HORAS, font=("Segoe UI", 10),
                                    state="readonly", width=8)
        combo_inicio.pack(side="left", padx=(6, 0), ipady=4)

        tk.Label(fila_hora, text="  →  Fin", font=("Segoe UI", 9),
                 bg=C["bg_card"], fg=C["text_3"]).pack(side="left", padx=(14, 0))

        var_fin = tk.StringVar(value=HORAS[4])      # 10:00 por defecto
        combo_fin = ttk.Combobox(fila_hora, textvariable=var_fin,
                                 values=HORAS, font=("Segoe UI", 10),
                                 state="readonly", width=8)
        combo_fin.pack(side="left", padx=(6, 0), ipady=4)

        # Actualizar opciones de fin cuando cambia inicio
        def _actualizar_horas_fin(*_):
            inicio_idx = HORAS.index(var_inicio.get())
            nuevas     = HORAS[inicio_idx + 1:]
            combo_fin.config(values=nuevas)
            if var_fin.get() not in nuevas:
                var_fin.set(nuevas[0] if nuevas else "")
        var_inicio.trace_add("write", _actualizar_horas_fin)
        _actualizar_horas_fin()
        ...
        ph_inicio = ""   # ya no se usa placeholder — Combobox
        ph_fin    = ""
        ...
        def _guardar():
            ...
            if _fecha_sel[0] is None:
                lbl_msg.config(text="Selecciona una fecha en el calendario.", fg=C["red"])
                return
            fecha  = str(_fecha_sel[0])   # YYYY-MM-DD
            inicio = var_inicio.get()
            fin    = var_fin.get()
            resp   = _leer(entry_resp, ph_resp)
            desc   = txt_desc.get("1.0", "end").strip()

            if not inicio:
                lbl_msg.config(text="Selecciona la hora de inicio.", fg=C["red"]); return
            if not fin:
                lbl_msg.config(text="Selecciona la hora de fin.", fg=C["red"]); return
            if not resp:
